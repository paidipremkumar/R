...
#b) creating decile lift chart
install.packages("gains")
library(gains)
gain <- gains(data.df$Actual, data.df$`Propensity of 1`)

heights <- gain$mean.resp/mean( data.df$`Propensity of 1`)
...
plot(c(0,gain$cume.pct.of.total*sum(valid.data$Coupon))~c(0,gain$cume.obs), 
     xlab="# cases", ylab="Cumulative", main="", type="l")
lines(c(0,sum(valid.data$Coupon))~c(0, dim(valid.data)[1]), lty=2)

heights <- gain$mean.resp/mean(valid.data$Coupon)
midpoints <- barplot(heights, names.arg = gain$depth, ylim = c(0,9), 
                     xlab = "Percentile", ylab = "Mean Response", main = "Decile-wise lift chart")
